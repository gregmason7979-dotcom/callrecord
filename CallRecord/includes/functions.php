		...
		/* 	mssql_select_db(dbname,$connect) ; */
                ...
                public function getAgentRoster()
                {
                        ...
                        foreach ($listFull as $valueFull) {
                                ...
                                $uniqueAgentIds[$agentIdForLookup] = $agentIdForLookup;
                        }
                        ...
                        if (!empty($uniqueAgentIds)) {
                                $placeholders = implode(', ', array_fill(0, count($uniqueAgentIds), '?'));
                                $sql = 'SELECT id, first_name, last_name FROM dbo.cc_user WHERE id IN (' . $placeholders . ')';
                                $params = array_values($uniqueAgentIds);
                                ...
                                if ($query !== false) {
                                        while ($row = sqlsrv_fetch_array($query, SQLSRV_FETCH_ASSOC)) {
                                                ...
                                                $agentLookup[$rawId] = $display;
                                                $agentLookup[$normalizedId] = $display;
                                        }
                        ...
                        foreach ($rawEntries as $entry) {
                                $valueFull = $entry['directory'];
                                $agentIdForLookup = $entry['agentId'];

                                $agentDisplay = isset($agentLookup[$agentIdForLookup]) ? $agentLookup[$agentIdForLookup] : $valueFull;
