		...
		<?php 
				...
				if(is_dir($subdirectory))
				{
					...
					foreach($list[0] as $value)
					 {
						...
						$clickTarget = "'http://192.168.1.154/SeCRecord/".$value_full."/".$value['file']."/".$model->replacePlus($uuval)."'";
						?>
					...
					if(is_array($new_array))
					{
						...
						$callid			=	$explode[4];
						$call			=	explode('.',$callid);
						$clickTarget = "'http://192.168.1.154/SeCRecord/".$value_full."/".$model->replacePlus($val)."'";
						?>
						<?php echo $model->renderRecordingRow(
						        $i,
						        $clickTarget,
